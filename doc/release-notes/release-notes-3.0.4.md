TRAFICOIN Core version 3.0.4 is now available from:
...
TRAFICOIN Core v3.0.4 is a mandatory update for all users. This release contains various updates/fixes pertaining to the zTRFC protocol, supply tracking, block transmission and relaying, as well as usability and quality-of-life updates to the GUI.
...
- #294 `27c0943` Add additional checks for txid for ztrfc spend. (presstab)
...
- #327 `aa1235a` [Main] Don't limit zTRFC spends from getting into the mempool (Fuzzbawls)
...
### GUI
- #309 `f560ffc` [UI] Better error message when too much inputs are used for spending zTRFC (Mrs-X)
...
- #314 `f1c830a` Fix issue causing crash when traficoind --help was invoked (Jon Spock)
- #326 `8b6a13e` Combine 2 LogPrintf statement to reduce debug.log clutter (Jon Spock)
- #328 `a6c18c8` [Main] TRAFICOIN not responding on user quitting app (Aaron Langford)
