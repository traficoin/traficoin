TRAFICOIN Core version *3.1.0* is now available from:  <https://github.com/traficoin-project/traficoin/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/traficoin-project/traficoin/issues>


Mandatory Update
==============

TRAFICOIN Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zTRFC Staking is here! zTRFC staking will be activated on the morning of the 8th of May 2018. With the release of zTRFC staking, there are effectively 2 versions of zTRFC, zTRFC minted on the 3.0.6 TRAFICOIN wallet or lower, and zTRFC minted on TRAFICOIN wallet version or higher. New features in this release will require the use of zTRFC v2, zTRFC minted on this wallet release 3.1.0 or later. If you currently hold zTRFC v1 and wish to take advantage of zTRFC staking and deterministic zTRFC, you will need to spend the zTRFC v1 to yourself and remint zTRFC v2.
Note: To find your zTRFC version, click the privacy tab, then the zTRFC Control button then expand the arrows next to the desired denomination.


### Deterministic zTRFC Seed Keys

zTRFC is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zTRFC outside of the wallet that the zTRFC had been minted on. zTRFC can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zTRFC minting

zTRFC minting now only requires 1 further mint (down from 2) to mature. zTRFC mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zTRFC Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zTRFC denomination has been spent or not.



TRFC/zTRFC Staking and Masternode Rewards
--------------

### TRFC, zTRFC and Masternode Payment Schedule

To encourage the use of zTRFC and increase the TRAFICOIN zerocoin anonymity set, the TRAFICOIN payment schedule has been changed to the following:

If a user staking zTRFC wins the reward for their block, the following zTRFC reward will be: 
- 3 zTRFC (3 x 1 denominations) rewarded to the staker, 2 TRFC rewarded to the masternode owner and 1 TRFC available for the budget. This is a total block reward of 6 TRFC, up from 5.

If a user staking TRFC wins the reward, the following amounts will be rewarded: 
- 2 TRFC to the TRFC staker, 3 TRFC to the Masternode owner and 1 TRFC available for the budget. This is a total block reward of 6 TRFC, up from 5.


### Return change to sender when minting zTRFC

Previously, zTRFC minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most TRFC and return the change from a zTRFC mint to that address.
...
The visual layout of the TRAFICOIN Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zTRFC minting feature. This is enabled by default and the enablezeromint=0 setting in the traficoin.conf file will overwrite the GUI option.
-	The percentage of autominted zTRFC can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular TRFC.
...
### Summary
 ...
 The TRAFICOIN wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the traficoin.conf file, found in the TRAFICOIN data directory.
- backuppath = <directory / full path>
- ztrfcbackuppath = <directory / full path>
...
* If zTRFC backup, auto generated name is `wallet-autoztrfcbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
TRAFICOIN Daemon & Client (RPC Changes)
--------------
...
This allows running traficoind without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the TRAFICOIN support group, which often times sees users say "I didn't spend that zTRFC". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the TRAFICOIN masternodes, the monthly budget available to be utilised has been increased to 42,000 TRFC / month. This TRFC only has the opportunity to be raised once per month (paid to winning proposals) with any unused TRFC not created by the blockchain.

### Budget Finalization Fee

The TRAFICOIN finalization fee for successful proposals has now been reduced, this fee is now 5 TRFC down from 50 TRFC. The total fee outlay for a successful proposal is now a total of 55 TRFC.
...
-	Enforced v1 zTRFC spends to require a security level of 100
-	Updates to zTRFC spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the TRAFICOIN-Qt masternodes tab
-	Updated TRAFICOIN-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and TRAFICOIN's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zTRFC Control dialog in the QT wallet, a user is able to see zTRFC marked as version 1 or 2.

zPoS (zTRFC staking)
--------------

Once a zTRFC has over 200 confirmations it becomes available to stake. Staking zTRFC will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zTRFC. So for example if a 1,000 zTRFC denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zTRFCs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within TRAFICOIN's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zTRFC, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zTRFC that can be recalculated at any time using the seed. Deterministic zTRFC allows for users to backup all of their future zTRFC by simply recording their seed and keeping it in a safe place (similar to backing up a private key for TRFC). The zTRFC seed needs to remain in the wallet in order to spend the zTRFC after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zTRFC data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

TRAFICOIN's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which TRAFICOIN used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to TRAFICOIN using the bug bounty program. The modulus is now correctly imported and TRAFICOIN's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zTRFC v2: zPoS dzTRFC ezTRFC external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zTRFC balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zTRFC status output (Mrs-X)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault traficoin-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zTRFC spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zTRFC. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zTRFC. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/traficoin-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
