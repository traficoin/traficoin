...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered traficoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 TRFC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal traficoin:URI (traficoin:XyZ...?message=example)
            {
...
static void NotifyzTRFCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzTRFCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzTRFCReset.connect(boost::bind(NotifyzTRFCReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzTRFCReset.disconnect(boost::bind(NotifyzTRFCReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->ztrfcTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
