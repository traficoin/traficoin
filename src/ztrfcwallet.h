...
#ifndef TRAFICOIN_ZTRFCWALLET_H
#define TRAFICOIN_ZTRFCWALLET_H
...
public:
    CzTRFCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZTRFC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZTRFC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //TRAFICOIN_ZTRFCWALLET_H
