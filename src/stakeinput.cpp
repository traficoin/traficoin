...
// The zTRFC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zTRFC is a hash of the serial
...
{
    //Create an output returning the zTRFC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZTRFCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zTRFC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zTRFC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZTRFCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zTRFC output", __func__);
...
{
    CzTRFCTracker* ztrfcTracker = pwallet->ztrfcTracker.get();
    CMintMeta meta;
    if (!ztrfcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    ztrfcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a TRFC stake is the outpoint
