...
public:
    ...
    bool WriteZTRFCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZTRFCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZTRFCSeed_deprecated(uint256& seed);
    bool EraseZTRFCSeed();
    bool EraseZTRFCSeed_deprecated();

    bool WriteZTRFCCount(const uint32_t& nCount);
    bool ReadZTRFCCount(uint32_t& nCount);
