...
CzTRFCWallet::CzTRFCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing ztrfc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTRFCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTRFCSeed_deprecated()) {
                    LogPrintf("%s: Updated zTRFC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old ztrfc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of ztrfc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzTRFCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTRFCCount(nCountLastUsed);
    else if (!walletdb.ReadZTRFCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTRFCWallet::Lock()
{
    seedMaster = 0;
}

void CzTRFCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTRFCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTRFC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTRFCWallet::LoadMintPoolFromDB()
{
...
void CzTRFCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTRFCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTRFCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->ztrfcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzTRFCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTRFC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to ztrfcTracker which also adds to database
    pwalletMain->ztrfcTracker->Add(dMint, true);
...
void CzTRFCWallet::SeedToZTRFC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzTRFCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTRFCWallet::GenerateDeterministicZTRFC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTRFCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTRFCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
