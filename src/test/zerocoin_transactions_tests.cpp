...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.ztrfcTracker = unique_ptr<CzTRFCTracker>(new CzTRFCTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZTRFC_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZTRFC_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
